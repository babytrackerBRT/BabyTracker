...
type EditKind = "feeding" | "diaper" | null;
...
function prettyType(e: EventRow) {
...
export default function DnevnikPage() {
  ...
  const filtered = useMemo(() => {
    ...
    const toTs = to ? new Date(to).getTime() + 24 * 60 * 60 * 1000 - 1 : null; // do kraja dana
  ...
  const grouped = useMemo(() => {
    ...
    for (const e of filtered) {
      const k = dayKeyFromIso(e.occurred_at);
  ...
  async function onDelete(e: EventRow) {
    const ok = confirm(`Obriši unos: ${prettyType(e)} u ${formatTime(e.occurred_at)}?`);
    if (!ok) return;
        ...
        <div className="grid grid-cols-1 gap-2">
          <div>
            ...
            <Input
              ...
              placeholder="npr. formula, mokra, osip..."
            />
          </div>
        ...
        grouped.map((g) => (
          <Card key={g.key}>
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-gray-500">{g.items.length} unosa</div>
            </div>
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <div className="font-semibold truncate">{prettyType(e)}</div>
          ...
          >
            <div className="mx-auto max-w-md rounded-2xl bg-white p-4 shadow-2xl dark:bg-gray-950">
              ...
              <div className="mt-1 text-xs text-gray-500">
                {prettyType(actionFor)} • {formatTime(actionFor.occurred_at)}
              </div>
            ...
            <div className="mx-auto max-w-md rounded-2xl bg-white p-4 shadow-2xl dark:bg-gray-950 max-h-[75vh] overflow-y-auto">
              ...
              {editKind === "feeding" && (
                <div className="mt-4 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <Button
                      variant={editFeedingMode === "formula" ? "primary" : "secondary"}
                      onClick={() => setEditFeedingMode("formula")}
                    >
                      Formula
                    </Button>
                    <Button
                      variant={editFeedingMode === "breast" ? "primary" : "secondary"}
                      onClick={() => setEditFeedingMode("breast")}
                    >
                      Dojenje
                    </Button>
                    <Button
                      variant={editFeedingMode === "solid" ? "primary" : "secondary"}
                      onClick={() => setEditFeedingMode("solid")}
                    >
                      Čvrsta
                    </Button>
                  </div>
              ...
              {editKind === "diaper" && (
                <div className="mt-4 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <Button
                      variant={editDiaperKind === "wet" ? "primary" : "secondary"}
                      onClick={() => setEditDiaperKind("wet")}
                    >
                      Mokra
                    </Button>
                    <Button
                      variant={editDiaperKind === "poop" ? "primary" : "secondary"}
                      onClick={() => setEditDiaperKind("poop")}
                    >
                      Velika
                    </Button>
                    <Button
                      variant={editDiaperKind === "mixed" ? "primary" : "secondary"}
                      onClick={() => setEditDiaperKind("mixed")}
                    >
                      Obe
                    </Button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      variant={editRash ? "primary" : "secondary"}
                      onClick={() => setEditRash((v) => !v)}
                    >
                      Osip: {editRash ? "Da" : "Ne"}
                    </Button>
                    <Button
                      variant={editCream ? "primary" : "secondary"}
                      onClick={() => setEditCream((v) => !v)}
                    >
