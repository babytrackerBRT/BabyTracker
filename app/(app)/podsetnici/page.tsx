...
import { createDailyVitaminDefinitionAndOccurrences, createOneOffReminder, listUpcomingOccurrences, markOccurrenceDone } from "@/lib/data/reminders";
import type { Baby, ReminderOccurrence } from "@/types/db";
import { formatDate, formatTime, randomToken } from "@/lib/utils";
...
export default function RemindersPage() {
  ...
  const filtered = useMemo(() => babyId ? items.filter(i => i.baby_id === babyId) : items, [items, babyId]);
  ...
  async function addOneOff() {
    if (!familyId) return;
    await createOneOffReminder(familyId, babyId || null, oneOffTitle, new Date(oneOffWhen).toISOString(), "doctor");
    await load();
  }
  ...
  const todayList = filtered.filter(i => new Date(i.scheduled_for).getTime() >= startOfToday && new Date(i.scheduled_for).getTime() < startOfTomorrow);
  const tomorrowList = filtered.filter(i => new Date(i.scheduled_for).getTime() >= startOfTomorrow && new Date(i.scheduled_for).getTime() < startOfDayAfter);
  const upcomingList = filtered.filter(i => new Date(i.scheduled_for).getTime() >= startOfDayAfter).slice(0, 20);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-2">
          <div className="rounded-xl border border-gray-200 p-3 dark:border-gray-800">
            <div className="text-xs font-semibold text-gray-500">Daily vitamin (kreira definiciju + 14 occurrences)</div>
            <div className="mt-2 grid grid-cols-2 gap-2">
              <Input value={title} onChange={(e) => setTitle(e.target.value)} />
              <Input value={time} onChange={(e) => setTime(e.target.value)} placeholder="18:00" />
            </div>
            <Button className="mt-2 w-full" onClick={addDailyVitamin}>Dodaj vitamins daily</Button>
          </div>
          ...
          <div className="rounded-xl border border-gray-200 p-3 dark:border-gray-800">
            ...
            <div className="mt-2 grid grid-cols-1 gap-2">
              ...
              <Input value={oneOffWhen} onChange={(e) => setOneOffWhen(e.target.value)} type="datetime-local" />
            </div>
            <Button variant="secondary" className="mt-2 w-full" onClick={addOneOff}>Dodaj jednokratni</Button>
          </div>
        </div>
        <div className="text-xs text-gray-500">Ovo je funkcionalno i odmah puni Supabase. UI će kasnije biti identičan mockup-u.</div>
      </Card>

      <Card>
        <div className="text-sm font-extrabold">Danas</div>
        <div className="mt-3 space-y-2">
          {todayList.length ? todayList.map(r => (
            <div key={r.id} className="flex items-center justify-between rounded-xl border border-gray-200 px-3 py-2 text-sm dark:border-gray-800">
              ...
              {r.status !== "done" ? <Button className="px-3 py-2" onClick={() => done(r.id)}>Završeno</Button> : <div className="text-xs font-semibold text-green-600">DONE</div>}
            </div>
      ...
      <Card>
        ...
        <div className="mt-3 space-y-2">
          {tomorrowList.length ? tomorrowList.map(r => (
            <div key={r.id} className="flex items-center justify-between rounded-xl border border-gray-200 px-3 py-2 text-sm dark:border-gray-800">
              <div className="min-w-0">
                <div className="truncate font-semibold">{r.title}</div>
                <div className="text-xs text-gray-500">{formatTime(r.scheduled_for)} • {r.category}</div>
      ...
      <Card>
        ...
        <div className="mt-3 space-y-2">
          {upcomingList.length ? upcomingList.map(r => (
            <div key={r.id} className="flex items-center justify-between rounded-xl border border-gray-200 px-3 py-2 text-sm dark:border-gray-800">
              <div className="min-w-0">
                <div className="truncate font-semibold">{r.title}</div>
                <div className="text-xs text-gray-500">{formatDate(r.scheduled_for)} • {formatTime(r.scheduled_for)} • {r.category}</div>
              </div>
              <div className="text-xs font-semibold text-gray-500">{r.status}</div>
            </div>
